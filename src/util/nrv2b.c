...
static void swd_initdict(struct ucl_swd* s, const uint8_t* dict, unsigned int dict_len)
...
static void swd_insertdict(struct ucl_swd* s, unsigned int node, unsigned int len)
{
    unsigned int key;
...
static int swd_init(struct ucl_swd* s, const uint8_t* dict, unsigned int dict_len)
{
    unsigned int i = 0;
...
static void swd_exit(struct ucl_swd* s)
{
    /* unused s */
    (void)s;
}
...
static __inline__ void swd_getbyte(struct ucl_swd* s)
{
    int c;
...
static __inline__ void swd_remove_node(struct ucl_swd* s, unsigned int node)
{
    if (s->node_count == 0)
    {
        unsigned int key;
...
static void swd_accept(struct ucl_swd* s, unsigned int n)
{
    assert(n <= s->look);
...
static void swd_search(struct ucl_swd* s, unsigned int node, unsigned int cnt)
{
...
static void swd_findbest(struct ucl_swd* s)
{
    unsigned int key;
...
static int
len_of_coded_match(struct ucl_compress* c, unsigned int m_len, unsigned int m_off)

{
    int b;
    if (m_len < 2 || (m_len == 2 && (m_off > M2_MAX_OFFSET)) || m_off > c->conf.max_offset)
        return -1;
    assert(m_off > 0);
    ...
    unsigned int* result)
{
    ...
    while (c->look > 0)
    {
        ...
        ahead = 0;
        if (SC_TRY_LAZY <= 0 || m_len >= SC_MAX_LAZY || m_off == c->last_m_off)
...
void Encode(void) /* compression */
{
    ...
    #define GETBIT_LE32(bb, src, ilen) \
        (bc > 0 ? ((bb >> --bc) & 1) : (bc = 31, bb = *(const uint32_t*)((src) + ilen), ilen += 4, (bb >> 31) & 1))

    #define GETBIT_LE64(bb, src, ilen) \
        (bc > 0 ? ((bb >> --bc) & 1) : (bc = 63, bb = *(const uint64_t*)((src) + ilen), ilen += 8, (bb >> 63) & 1))
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 4) {
        if ((s = argv[1], s[1] || strpbrk(s, "DEde") == NULL) || (s = argv[2], (infile = fopen(s, "rb")) == NULL) || (s = argv[3], (outfile = fopen(s, "wb")) == NULL)) {
            Fprintf((stderr, "??? %s\n", s));
            return EXIT_FAILURE;
        }
    }
