...
static void draw_info_row(struct settings_ui* ui) {
    ...
    setting_name((ui->row.origin ? ui->row.origin : ui->settings),
...
static void draw_instruction_row(struct settings_ui* ui) {
    ...
    if (ui->row.editing) {
        ...
        msg(INSTRUCTION_ROW,
            ...
            ((ui->row.origin == ui->settings) ? "Ctrl-D - delete setting" INSTRUCTION_PAD : ""));
    }
}
...
static int main_loop(struct settings* settings) {
    ...
    while (1) {
        ...
        if (redraw) {
            ...
            color_set((ui.row.editing ? CPAIR_EDIT : CPAIR_SELECT), NULL);
            draw_setting_row(&ui);
            color_set(CPAIR_NORMAL, NULL);
