...
static void _print_label(struct _softlabel sl) {
    ...
    if (!sl.label[0]) {
        ...
        if (sl.fmt == 1) {
            memset(str, space_ch,
                   (size_t)(slks->max_label_len - strlen(sl.label)) / 2);
        }
        if (sl.fmt == 2) {
            memset(str, space_ch,
                   (size_t)(slks->max_label_len - strlen(sl.label)));
        }
        strcat(str, sl.label);
...
int slk_colour(short colour_pair_number) {
    ...
    slks->attrs = ((unsigned short)colour_pair_number << CPAIR_SHIFT) | (slks->attrs & A_ATTRIBUTES);
