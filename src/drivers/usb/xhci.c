...
#define ECODE(code)                                                           \
    (((code) < 32)   ? EUNIQ(EINFO_EIO, ((code)&31), EIO_DATA, EIO_BABBLE,    \
                             ...
                             EIO_ISOCH)                                       \
     : ((code) < 64) ? EUNIQ(EINFO_EPROTO, ((code)&31), EPROTO_LOST,          \
...
static void xhci_transfer(struct xhci_device* xhci,
    ...
    if (!((trb->code == XHCI_CMPLT_SUCCESS) ||
        ...
        /* Sanity check */
        assert((endpoint->context->state & XHCI_ENDPOINT_STATE_MASK) != XHCI_ENDPOINT_RUNNING);
...
static int xhci_endpoint_stream(struct usb_endpoint* ep,
    ...
    /* Map I/O buffer */
    if ((rc = iob_map(iobuf, xhci->dma, len,
                      ((ep->address & USB_DIR_IN) ? DMA_RX : DMA_TX))) != 0)
        goto err_map;
    data = iob_dma(iobuf);
