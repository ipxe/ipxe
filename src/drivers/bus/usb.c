...
usb_interface_association_descriptor(struct usb_configuration_descriptor* config,
                                     unsigned int first) {
...
const char* usb_endpoint_name(struct usb_endpoint* ep) {
    ...
    snprintf(buf, sizeof(buf), "EP%d%s",
             ...
             (address ? ((address & USB_ENDPOINT_IN) ? " IN" : " OUT") : ""));
    return buf;
}
...
struct usb_driver* usb_find_driver(struct usb_function_descriptor* desc,
    ...
    for_each_table_entry(driver, USB_DRIVERS) {
        for (i = 0; i < driver->id_count; i++) {
            ...
            if ((driver->class.class.scalar ^ desc->class.scalar) & driver->class.mask.scalar)
                continue;
...
static int register_usb(struct usb_device* usb) {
    ...
    mtu = ((protocol < USB_PROTO_3_0) ? usb->device.mtu : (1 << usb->device.mtu));
...
unsigned int usb_route_string(struct usb_device* usb) {
    ...
    for (route = 0; (parent = usb->port->hub->usb); usb = parent) {
        route <<= 4;
        route |= ((usb->port->address > 0xf) ? 0xf : usb->port->address);
    }
