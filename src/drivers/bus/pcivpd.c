    ...
    while (*len >= sizeof(read_field)) {
        ...
        *len -= sizeof(read_field);
        ...
        if (read_field.keyword == PCI_VPD_KEYWORD(field)) {
            *len = read_field.len;
            DBGC(vpd, PCI_FMT " VPD field " PCI_VPD_FIELD_FMT " is at [%04x,%04zx)\n", PCI_ARGS(vpd->pci),
...
int pci_vpd_resize(struct pci_vpd* vpd, unsigned int field, size_t len,
    ...
    if ((rc = pci_vpd_find(vpd, field, &old_address,
        ...
        if (old_address > rw_address) {
            DBGC(vpd, PCI_FMT " VPD field " PCI_VPD_FIELD_FMT " at [%04x,%04zx) is after field " PCI_VPD_FIELD_FMT " at [%04x,%04zx)\n",
                 PCI_ARGS(vpd->pci),
                 PCI_VPD_FIELD_ARGS(field),
        ...
        /* Calculate available length */
        available_len = ((rw_len > sizeof(new_field)) ? (rw_len - sizeof(new_field)) : 0);
    }

    /* Dump region before changes */
    dump_address = dst_address;
    dump_len = (rw_address + rw_len - dump_address);
    DBGC(vpd, PCI_FMT " VPD before resizing field " PCI_VPD_FIELD_FMT " to %zd bytes:\n", PCI_ARGS(vpd->pci),
    ...
    if (len > available_len) {
        DBGC(vpd, PCI_FMT " VPD no space for field " PCI_VPD_FIELD_FMT " (need %02zx, have %02zx)\n",
             PCI_ARGS(vpd->pci), PCI_VPD_FIELD_ARGS(field),
             len, available_len);
        rc = -ENOSPC;
        goto err_no_space;
    ...
    if (len) {
        ...
        *address = dst_address;
        ...
        DBGC(vpd, PCI_FMT " VPD field " PCI_VPD_FIELD_FMT " no longer exists\n", PCI_ARGS(vpd->pci),
             PCI_VPD_FIELD_ARGS(field));
    }
    ...
    DBGC(vpd, PCI_FMT " VPD after resizing field " PCI_VPD_FIELD_FMT " to %zd bytes:\n", PCI_ARGS(vpd->pci),
         PCI_VPD_FIELD_ARGS(field), len);
    pci_vpd_dump(vpd, dump_address, dump_len);
