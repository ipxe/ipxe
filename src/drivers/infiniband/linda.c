...
static const char* linda_link_state_text(unsigned int link_state) {
    switch (link_state) {
        case LINDA_LINK_STATE_DOWN:
            return "DOWN";
        case LINDA_LINK_STATE_INIT:
            return "INIT";
        case LINDA_LINK_STATE_ARM:
            return "ARM";
        case LINDA_LINK_STATE_ACTIVE:
            return "ACTIVE";
        case LINDA_LINK_STATE_ACT_DEFER:
            return "ACT_DEFER";
        default:
            return "UNKNOWN";
    }
...
static int linda_post_recv(struct ib_device* ibdev,
    ...
    switch (LINDA_RECV_PAYLOAD_SIZE) {
        case 2048:
            bufsize = LINDA_EAGER_BUFFER_2K;
            break;
        case 4096:
            bufsize = LINDA_EAGER_BUFFER_4K;
            break;
        case 8192:
            bufsize = LINDA_EAGER_BUFFER_8K;
            break;
        case 16384:
            bufsize = LINDA_EAGER_BUFFER_16K;
            break;
        case 32768:
            bufsize = LINDA_EAGER_BUFFER_32K;
            break;
        case 65536:
            bufsize = LINDA_EAGER_BUFFER_64K;
            break;
        default:
    ...
    BIT_FILL_1(&rcvegrindexhead,
               Value, ((linda_wq->eager_prod + 1) & (linda_wq->eager_entries - 1)));
    linda_writeq_array64k(linda, &rcvegrindexhead,
                          QIB_7220_RcvEgrIndexHead0_offset, ctx);
...
static int linda_ib_epb_ram_xfer(struct linda* linda, unsigned int address,
    ...
    while (len) {
        ...
        if ((address % LINDA_EPB_UC_CHUNK_SIZE) == 0) {
            ...
            control = (read ? LINDA_EPB_UC_CTL_READ : LINDA_EPB_UC_CTL_WRITE);
...
static int linda_init_ib_serdes(struct linda* linda) {
    ...
    BIT_FILL_6(&ibcctrl, /* Tuning values taken from Linux driver */
               ...
               MaxPktLen, ((LINDA_RECV_HEADER_SIZE + LINDA_RECV_PAYLOAD_SIZE + 4 /* ICRC */) >> 2),
               PhyerrThreshold, 0xf,
