...
static const char* qib7322_link_state_text(unsigned int link_state) {
    switch (link_state) {
        case QIB7322_LINK_STATE_DOWN:
            return "DOWN";
        case QIB7322_LINK_STATE_INIT:
            return "INIT";
        case QIB7322_LINK_STATE_ARM:
            return "ARM";
        case QIB7322_LINK_STATE_ACTIVE:
            return "ACTIVE";
        case QIB7322_LINK_STATE_ACT_DEFER:
            return "ACT_DEFER";
        default:
            return "UNKNOWN";
    }
...
static int qib7322_create_recv_wq(struct ib_device* ibdev,
    ...
    DBGC(qib7322, "QIB7322 %p port %d QPN %ld CTX %d hdrs [%lx,%lx) prod "
         ...
         (virt_to_bus(qib7322_wq->header) + QIB7322_RECV_HEADERS_SIZE),
...
static int qib7322_post_recv(struct ib_device* ibdev,
    ...
    switch (QIB7322_RECV_PAYLOAD_SIZE) {
        case 2048:
            bufsize = QIB7322_EAGER_BUFFER_2K;
            break;
        case 4096:
            bufsize = QIB7322_EAGER_BUFFER_4K;
            break;
        case 8192:
            bufsize = QIB7322_EAGER_BUFFER_8K;
            break;
        case 16384:
            bufsize = QIB7322_EAGER_BUFFER_16K;
            break;
        case 32768:
            bufsize = QIB7322_EAGER_BUFFER_32K;
            break;
        case 65536:
            bufsize = QIB7322_EAGER_BUFFER_64K;
            break;
        default:
            linker_assert(0, invalid_rx_payload_size);
    ...
    BIT_FILL_1(&rcvegrindexhead,
               Value, ((qib7322_wq->eager_prod + 1) & (qib7322_wq->eager_entries - 1)));
    qib7322_writeq_array64k(qib7322, &rcvegrindexhead,
                            QIB_7322_RcvEgrIndexHead0_offset, ctx);
...
static int qib7322_init_ib_serdes(struct qib7322* qib7322) {
    ...
    BIT_FILL_5(&ibcctrla, /* Tuning values taken from Linux driver */
               ...
               MaxPktLen, ((QIB7322_RECV_HEADER_SIZE + QIB7322_RECV_PAYLOAD_SIZE + 4 /* ICRC */) >> 2),
               PhyerrThreshold, 0xf,
