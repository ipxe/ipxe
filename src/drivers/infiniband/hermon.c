...
#define HERMON_MOD_STAT_CFG_OFFSET(field) \
    ((MLX_BIT_OFFSET(struct hermonprm_mod_stat_cfg_st, field) / 8) & ~(sizeof(struct hermonprm_scalar_parameter) - 1))
...
static void hermon_free_qpn(struct ib_device* ibdev,
    ...
    qpn_offset = ((qp->qpn & ~HERMON_QPN_RANDOM_MASK) - hermon->qpn_base);
    if (qpn_offset >= 0)
...
static unsigned int hermon_sched_queue(struct ib_device* ibdev,
                                       struct ib_queue_pair* qp) {
    return (((qp->type == IB_QPT_SMI) ? HERMON_SCHED_QP0 : HERMON_SCHED_DEFAULT) |
            ((ibdev->port - 1) << 6));
}
...
static int hermon_modify_qp(struct ib_device* ibdev,
    ...
    if (hermon_qp->state < HERMON_QP_ST_RTR) {
        ...
        MLX_FILL_2(&qpctx, 4,
                   qpc_eec_data.mtu,
                   ((qp->type == IB_QPT_ETH) ? HERMON_MTU_ETH : HERMON_MTU_2048),
...
static unsigned int (*hermon_fill_send_wqe[])(struct ib_device* ibdev,
                                              struct ib_queue_pair* qp,
                                              struct ib_address_vector* dest,
                                              struct io_buffer* iobuf,
                                              union hermon_send_wqe* wqe) = {
