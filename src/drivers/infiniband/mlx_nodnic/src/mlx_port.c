...
#define NODIC_RING_QP_ADDR_LOW 0x4
    ...
    status = nodnic_cmd_write(device_priv, (*qp)->send.nodnic_ring.offset + NODIC_RING_QP_ADDR_HIGH,
    ...
    status = nodnic_cmd_write(device_priv, (*qp)->send.nodnic_ring.offset + NODIC_RING_QP_ADDR_LOW,
                              (mlx_uint32)address);
    MLX_FATAL_CHECK_STATUS(status, write_send_addr_err,
                           "send address write error 2");
    /* update receive ring */
    address = (mlx_uint64)(*qp)->receive.nodnic_ring.wqe_physical;
    status = nodnic_cmd_write(device_priv, (*qp)->receive.nodnic_ring.offset + NODIC_RING_QP_ADDR_HIGH,
                              (mlx_uint32)(address >> 32));
    ...
    address = address | log_size;
    status = nodnic_cmd_write(device_priv, (*qp)->receive.nodnic_ring.offset + NODIC_RING_QP_ADDR_LOW,
                              (mlx_uint32)address);
    MLX_FATAL_CHECK_STATUS(status, write_recv_addr_err,
    ...
    IN mlx_uint8 port_index)
{
    ...
    if (device_priv->device_cap.crspace_doorbells) {
        ...
        if (status != MLX_SUCCESS) {
            ...
            port_priv->data_flow_gw = (nodnic_port_data_flow_gw*)(device_priv->utils->config + offset);
        }
