    ...
    OUT mlx_uint32* cap_offset)
{
    ...
    while (cap_pointer != 0) {
        ...
        MLX_CHECK_STATUS(utils, status, check_err, "failed to check capability id");
        ...
        status = mlx_pci_read(utils, MlxPciWidthUint8, cap_pointer + PCI_GW_CAPABILITY_NEXT_POINTER_OFFSET,
                              1, &cap_pointer);
        MLX_CHECK_STATUS(utils, status, read_err,
                         "failed to read capability pointer");
    }
