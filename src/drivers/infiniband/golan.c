...
static inline int golan_provide_pages(struct golan* golan, uint32_t pages, __be16 func_id, struct golan_firmware_area* fw_area) {
    struct mbox* mailbox;
    int size_ibox = 0;
    int size_obox = 0;
    int rc = 0;
                           ...
                           struct ib_completion_queue* cq)
{
    ...
    for (i = 0; i < cq->num_cqes; ++i) {
        golan_cq->cqes[i].op_own = ((GOLAN_CQE_OPCODE_NOT_VALID << GOLAN_CQE_OPCODE_BIT) |
                               ...
                               int* qpn)
{
    ...
    in->ctx.flags = cpu_to_be32((golan_qp_type_to_st(qp->type)
                                 << GOLAN_QP_CTX_ST_BIT) |
                                (GOLAN_QP_PM_MIGRATED << GOLAN_QP_CTX_PM_STATE_BIT));
                           ...
                           struct io_buffer* iobuf)
{
    ...
    ctrl->opmod_idx_opcode = cpu_to_be32(GOLAN_SEND_OPCODE |
                                         ((u32)(golan_qp->sq.next_idx) << GOLAN_WQE_CTRL_WQE_IDX_BIT));
    ...
    wmb();
    writeq(*((__be64*)ctrl), golan->uar.virt + ((golan_qp->sq.next_idx & 0x1) ? DB_BUFFER0_EVEN_OFFSET
