...
static int golan_crusoe_set_hca_cap_payload ( struct golan *golan,
					       uint16_t opmod,
					       ...
					       const char *name )
{
	...
	cmd = write_cmd ( golan, DEF_CMD_IDX, GOLAN_CMD_OP_SET_HCA_CAP, opmod,
			  MEM_MBOX, NO_MBOX, ( 16 + 4096 ),
			  sizeof ( struct golan_cmd_set_hca_cap_mbox_out ) );
	mailboxes = GET_INBOX ( golan, MEM_MBOX );
	memset ( mailboxes, 0, GOLAN_MBOX_ARENA_SIZE );
	memcpy ( mailboxes[0].mblock.bdata, words, 256 );
	for ( i = 0 ; i < 8 ; i++ ) {
	...
	rc = send_command_and_wait ( golan, DEF_CMD_IDX, MEM_MBOX, NO_MBOX,
				     name );
	printf ( "Crusoe mlx5e VF: %s rc=%d status=0x%x syndrome=0x%x\n",
		 name, rc, ( ( struct golan_outbox_hdr * ) cmd->out )->status,
...
static inline int golan_crusoe_set_hca_cap_current ( struct golan *golan )
{
	...
	static const uint32_t linux_atomic_cap_words[] = {
	...
	static const uint32_t linux_odp_cap_words[] = {
		0x0, 0x0, 0x0, 0x0, 0xfc, 0x0, 0x80, 0xbc,
	...
	static const uint32_t linux_roce_cap_words[] = {
		0x207f, 0x0, 0x0, 0x0, 0x5000700, 0xb7120000, 0xc0ffff, 0xff000000,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
	...
	static const uint32_t linux_general2_cap_words[] = {
		0x0, 0x0, 0x0, 0x0, 0x87273528, 0xfe80fe80, 0x1898c040, 0xf0ff1f00,
		0x180000, 0x8f0f0000, 0x0, 0x507b401, 0x0, 0x16008903, 0x0, 0x8f040000,
		0x1040000, 0xff0000c0, 0x180800, 0x87878689, 0x86878988, 0x2000000, 0xb00, 0x5041e80,
		0x0, 0x0, 0x0, 0x0, 0x80a, 0x8003f3f, 0x0, 0x0,
		0xaa0200c1, 0x400000, 0x1c000015, 0x1000020, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
	};
	...
	if ( rc )
		return rc;
	if (( rc = golan_crusoe_set_hca_cap_payload ( golan, 6,
						      linux_atomic_cap_words,
						      "golan_crusoe_set_atomic_cap" ) ))
		return rc;
	if (( rc = golan_crusoe_set_hca_cap_payload ( golan, 4,
						      linux_odp_cap_words,
						      "golan_crusoe_set_odp_cap" ) ))
		return rc;
	if (( rc = golan_crusoe_set_hca_cap_payload ( golan, 8,
						      linux_roce_cap_words,
						      "golan_crusoe_set_roce_cap" ) ))
		return rc;
	if (( rc = golan_crusoe_set_hca_cap_payload ( golan, 64,
						      linux_general2_cap_words,
						      "golan_crusoe_set_general2_cap" ) ))
		return rc;
	GOLAN_PRINT_RC_AND_CMD_STATUS;
	return rc;
}
...
static int golan_crusoe_eth_transmit ( struct net_device *netdev,
	...
	*( ( __be32 * ) mlx5e->sq_dbr ) = cpu_to_be32 ( mlx5e->sq_prod );
	 ...
	 * Keep that exact offset and use the same one raw 64-bit store.
	 */
	writeq ( *( ( u64 * ) &wqe->ctrl ),
