...
static void atl1e_clean_tx_ring(struct atl1e_adapter* adapter)
{
    ...
    /* Zero out Tx-buffers */
    memset(tx_ring->desc, 0, sizeof(struct atl1e_tpd_desc) * ring_count);
    memset(tx_ring->tx_buffer, 0, sizeof(struct atl1e_tx_buffer) * ring_count);
}
...
static void atl1e_cal_ring_size(struct atl1e_adapter* adapter, u32* ring_size)
{
    *ring_size = ((u32)(adapter->tx_ring.count *
                        ...
                        7
                        /* tx ring, qword align */
                        + adapter->rx_ring.real_page_size * AT_PAGE_NUM_PER_QUEUE + 31
...
static void atl1e_init_ring_resources(struct atl1e_adapter* adapter)
{
    ...
    rx_ring->real_page_size = adapter->rx_ring.page_size + MAX_FRAME_SIZE + ETH_HLEN + VLAN_HLEN + ETH_FCS_LEN;
...
static int atl1e_setup_ring_resources(struct atl1e_adapter* adapter)
{
    ...
    tx_ring->desc = (struct atl1e_tpd_desc*)(adapter->ring_vir_addr + offset);
...
static inline void atl1e_configure_rx(struct atl1e_adapter* adapter)
{
    ...
    if (hw->nic_type != athr_l2e_revB) {
        AT_WRITE_REGW(hw, REG_RXQ_JMBOSZ_RRDTIM,
                      (u16)((RX_JUMBO_THRESH & RXQ_JMBOSZ_TH_MASK) << RXQ_JMBOSZ_TH_SHIFT |
...
static void atl1e_setup_mac_ctrl(struct atl1e_adapter* adapter)
{
    ...
    value |= ((u32)((SPEED_1000 == adapter->link_speed) ? MAC_CTRL_SPEED_1000 : MAC_CTRL_SPEED_10_100) << MAC_CTRL_SPEED_SHIFT);
...
static int atl1e_configure(struct atl1e_adapter* adapter)
{
    ...
    AT_WRITE_REG(hw, REG_MASTER_CTRL, MASTER_CTRL_LED_MODE | MASTER_CTRL_ITIMER_EN | MASTER_CTRL_ITIMER2_EN);
    ...
    /* 8. set MTU */
    AT_WRITE_REG(hw, REG_MTU, MAX_FRAME_SIZE + ETH_HLEN + VLAN_HLEN + ETH_FCS_LEN);
...
static inline u16 atl1e_tpd_avail(struct atl1e_adapter* adapter)
{
    ...
    return (u16)(next_to_clean > next_to_use) ? (next_to_clean - next_to_use - 1) : (tx_ring->count + next_to_clean - next_to_use - 1);
}
                         ...
                         struct io_buffer* iob, struct atl1e_tpd_desc* tpd)
{
    ...
    tpd->word2 = ((tpd->word2 & ~TPD_BUFLEN_MASK) |
                  ((cpu_to_le32(buf_len) & TPD_BUFLEN_MASK) << TPD_BUFLEN_SHIFT));
...
int atl1e_reset_hw(struct atl1e_hw* hw)
{
    ...
    if ((pci_cfg_cmd_word & (PCI_COMMAND_IO | PCI_COMMAND_MEM |
                             PCI_COMMAND_MASTER)) != (PCI_COMMAND_IO | PCI_COMMAND_MEM |
                                                      PCI_COMMAND_MASTER)) {
