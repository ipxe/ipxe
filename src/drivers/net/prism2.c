    ...
    const char* p)  /* Packet */
{
    ...
    if (HFA384x_EVSTAT_ISTXEXC(result)) {
        ...
        if (HFA384x_TXSTATUS_ISACKERR(status)) {
            printf(" ...acknowledgement error\n");
        }
        if (HFA384x_TXSTATUS_ISFORMERR(status)) {
            printf(" ...format error\n");
        }
        if (HFA384x_TXSTATUS_ISDISCON(status)) {
            printf(" ...disconnected error\n");
        }
        if (HFA384x_TXSTATUS_ISAGEDERR(status)) {
            printf(" ...AGED error\n");
        }
        if (HFA384x_TXSTATUS_ISRETRYERR(status)) {
            printf(" ...retry error\n");
        }
        return; /* fail */
    }
...
static int prism2_probe(struct nic* nic, hfa384x_t* hw) {
    ...
    for (tmp16 = 0; tmp16 < sizeof(hardcoded_ssid); tmp16++) {
        ssid[2 + tmp16] = hardcoded_ssid[tmp16];
    }
    ...
    do {
        ...
        if (!hfa384x_wait_for_event(hw, HFA384x_EVSTAT_INFO, 0, 1000, 2000, "Info event"))
            return 0;
        printf("done\n");
