...
static int bnxt_get_pci_info ( struct bnxt *bp )
{
	...
	pci_write_word16 ( bp->pdev, PCI_COMMAND, cmd_reg );
...
u32 bnxt_er_get_reg_val ( struct bnxt *bp, u32 reg_addr, u32 reg_type, u32 mask )
{
	...
	switch ( reg_type ) {
		case ER_QCFG_FW_HEALTH_REG_ADDR_SPACE_PCIE_CFG:
			pci_read_dword ( bp->pdev, reg_addr & mask, &reg_val );
			break;
...
void bnxt_rst_reg_val ( struct bnxt *bp, u32 reg_addr, u32 reg_val )
{
	...
	switch ( reg_type ) {
		case ER_QCFG_FW_HEALTH_REG_ADDR_SPACE_PCIE_CFG:
			pci_write_dword ( bp->pdev, reg_addr & mask, reg_val );
			break;
		case ER_QCFG_FW_HEALTH_REG_ADDR_SPACE_GRC:
...
void bnxt_hb_task ( struct bnxt* bp )
{
	u32 present_hb_cnt;
	unsigned short pci_command, new_command;
	u8 i;
...
static void bnxt_check_timer ( struct bnxt* bp )
{
	bp->er.curr_tick = currticks ( );

	if ( ( bp->er.curr_tick - bp->er.last_tick ) > 
			TICKS_PER_SEC * ( bp->er.drv_poll_freq / 10 ) ) {
		bnxt_hb_task ( bp );
		bp->er.last_tick = bp->er.curr_tick;
	}
}
