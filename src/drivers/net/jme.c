...
read_again:
    jwrite32(jme, JME_SMI, SMI_OP_REQ | smi_phy_addr(phy) | smi_reg_addr(reg));
               ...
               int phy, int reg, int val)
{
    ...
    jwrite32(jme, JME_SMI, SMI_OP_WRITE | SMI_OP_REQ | ((val << SMI_DATA_SHIFT) & SMI_DATA_MASK) | smi_phy_addr(phy) | smi_reg_addr(reg));
...
jme_reset_phy_processor(struct jme_adapter* jme)
{
    ...
    jme_mdio_write(jme->mii_if.dev,
                   jme->mii_if.phy_id,
                   MII_ADVERTISE, ADVERTISE_ALL | ADVERTISE_PAUSE_CAP | ADVERTISE_PAUSE_ASYM);
...
jme_enable_tx_engine(struct jme_adapter* jme)
{
     ...
     */
    wmb();
    jwrite32(jme, JME_TXCS, jme->reg_txcs | TXCS_SELECT_QUEUE0 | TXCS_ENABLE);
}
...
jme_enable_rx_engine(struct jme_adapter* jme)
{
     ...
     * Select Queue 0
     */
    jwrite32(jme, JME_RXCS, jme->reg_rxcs | RXCS_QUEUESEL_Q0);
    wmb();
     ...
     */
    wmb();
    jwrite32(jme, JME_RXCS, jme->reg_rxcs | RXCS_QUEUESEL_Q0 | RXCS_ENABLE | RXCS_QST);
}
...
jme_check_link(struct net_device* netdev, int testonly)
{
    ...
    if (phylink & PHY_LINK_UP) {
        ...
        if (phylink & PHY_LINK_DUPLEX) {
            ...
            jwrite32(jme, JME_TXMCS, TXMCS_DEFAULT | TXMCS_BACKOFF | TXMCS_CARRIERSENSE | TXMCS_COLLISION);
            jwrite32(jme, JME_TXTRHD, TXTRHD_TXPEN | ((0x2000 << TXTRHD_TXP_SHIFT) & TXTRHD_TXP) | TXTRHD_TXREN | ((8 << TXTRHD_TXRL_SHIFT) & TXTRHD_TXRL));
        }
        ...
        DBG("Link is up at %d Mbps, %s-Duplex, MDI%s.\n",
            ((phylink & PHY_LINK_SPEED_MASK) == PHY_LINK_SPEED_1000M) ? 1000 : ((phylink & PHY_LINK_SPEED_MASK) == PHY_LINK_SPEED_100M) ? 100
                                                                                                                                        : 10,
