...
static void natsemi_init_eeprom(struct natsemi_nic* natsemi) {
    ...
    natsemi->spibit.endianness =
        ((natsemi->flags & NATSEMI_EEPROM_LITTLE_ENDIAN) ? SPI_BIT_LITTLE_ENDIAN : SPI_BIT_BIG_ENDIAN);
...
static int natsemi_hwaddr(struct natsemi_nic* natsemi, void* hw_addr) {
    ...
    extract = ((natsemi->flags & NATSEMI_EEPROM_INSANE) ? natsemi_hwaddr_insane : natsemi_hwaddr_sane);
...
static int natsemi_create_ring(struct natsemi_nic* natsemi,
    ...
    offset = ((natsemi->flags & NATSEMI_64BIT) ? 0 : offsetof(typeof(desc[i].d32pad), d32));
...
static int natsemi_open(struct net_device* netdev) {
    ...
    writel((NATSEMI_TXCFG_CSI | NATSEMI_TXCFG_HBI | NATSEMI_TXCFG_ATP |
           ...
           (natsemi->regs + ((natsemi->flags & NATSEMI_64BIT) ? NATSEMI_TXCFG_64 : NATSEMI_TXCFG_32)));
    ...
    writel((NATSEMI_RXCFG_ARP | NATSEMI_RXCFG_ATX | NATSEMI_RXCFG_ALP |
           ...
           (natsemi->regs + ((natsemi->flags & NATSEMI_64BIT) ? NATSEMI_RXCFG_64 : NATSEMI_RXCFG_32)));
