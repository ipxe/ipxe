                                  ...
                                  struct io_buffer* iob)
{
    ...
    request = &priv->receive_post_ring[receives_posted & priv->receive_post_ring_wrap];
    request->addr_high = 0;
    wmb();
                            ...
                            uint32 data[3])
{
    ...
    command->response_addr.low = htonl(virt_to_bus(&priv->dma->command_response));
    for (i = 0; i < 9; i++)
        command->pad[i] = 0;
    wmb();
    command->pad[9] = 0;
    ...
    *priv->irq_deassert = 0; /* any value is OK. */
    ...
    if (valid & 2) {
        ...
        while (priv->transmits_done != nic_done_count) {
            ...
            iob = priv->transmit_iob[priv->transmits_done & MYRI10GE_TRANSMIT_WRAP];
            DBG2("%p ", iob);
...
static void myri10ge_net_close(struct net_device* netdev)
{
    ...
    while (priv->receives_done != priv->receives_posted) {
        free_iob(priv->receive_iob[priv->receives_done & MYRI10GE_RECEIVE_WRAP]);
        ++priv->receives_done;
    }
...
#define TRY(prefix, base, suffix)                            \
    ...
    data[0] = ((uint32_t)(sizeof(priv->dma->receive_completion)) | MXGEFW_CMD_SET_INTRQ_SIZE_FLAG_NO_STRICT_SIZE_CHECK);
    ...
    *((uint32*)(membase + data[0])) = 0;
    ...
    data[0] = (netdev->ll_addr[0] << 24 | netdev->ll_addr[1] << 16 | netdev->ll_addr[2] << 8 | netdev->ll_addr[3]);
    data[1] = ((netdev->ll_addr[4] << 8) | netdev->ll_addr[5]);
    ...
    priv->transmit_ring_wrap = data[0] / sizeof(mcp_kreq_ether_send_t) - 1;
    if (priv->transmit_ring_wrap & (priv->transmit_ring_wrap + 1)) {
        rc = -EPROTO;
        dbg = "TX_RING";
        goto abort_with_dma;
    }
    ...
    if (priv->receive_post_ring_wrap & (priv->receive_post_ring_wrap + 1)) {
        rc = -EPROTO;
        dbg = "RX_RING";
        goto abort_with_dma;
    }
...
static void myri10ge_net_poll(struct net_device* netdev)
{
    ...
    while (priv->receives_done != orig_receives_posted) {
        ...
        length = ntohs(dma->receive_completion
                           [priv->receives_done & MYRI10GE_RECEIVE_COMPLETION_WRAP]
                               .length);
        if (length == 0)
            break;
        ...
        iob = priv->receive_iob[priv->receives_done & MYRI10GE_RECEIVE_WRAP];
        iob_put(iob, length);
        ...
        dma->receive_completion[priv->receives_done & MYRI10GE_RECEIVE_COMPLETION_WRAP]
            .length = 0;
        wmb();
                                 ...
                                 struct io_buffer* iobuf)
{
    ...
    if (transmits_posted - priv->transmits_done > MYRI10GE_TRANSMIT_WRAP) {
        DBG("TX ring full\n");
        return -ENOBUFS;
    }
    ...
    kreq = &priv->transmit_ring[transmits_posted & priv->transmit_ring_wrap];
    kreq->addr_high = 0;
    ...
    ((uint32*)kreq)[3] = htonl(
        ...
        | 0x01 << 16                                                      /* rdma_count */
        | 0x00 << 8                                                       /* cksum_offset */
        | (MXGEFW_FLAGS_SMALL | MXGEFW_FLAGS_FIRST | MXGEFW_FLAGS_NO_TSO) /* flags */
    );
    wmb();
