...
static void tg3_aux_stat_to_speed_duplex(struct tg3* tp, u32 val, u16* speed, u8* duplex)
{
    ...
    switch (val & MII_TG3_AUX_STAT_SPDMASK) {
        ...
        default:
            if (tp->phy_flags & TG3_PHYFLG_IS_FET) {
                *speed = (val & MII_TG3_AUX_STAT_100) ? SPEED_100 : SPEED_10;
                *duplex = (val & MII_TG3_AUX_STAT_FULL) ? DUPLEX_FULL : DUPLEX_HALF;
                break;
            }
            *speed = SPEED_INVALID;
...
static void tg3_link_report(struct tg3* tp)
{
    ...
    if (!netdev_link_ok(tp->dev)) {
        ...
        DBGC(tp->dev, "Link is up at %d Mbps, %s duplex\n",
             (tp->link_config.active_speed == SPEED_1000 ? 1000 : (tp->link_config.active_speed == SPEED_100 ? 100 : 10)),
    ...
    if ((tp->link_config.autoneg == AUTONEG_ENABLE) && !force_reset &&
        ...
        if (new_bmcr != bmcr) {
            ...
            if (tp->link_up) {
                ...
                tg3_writephy(tp, MII_BMCR, bmcr | BMCR_ANRESTART | BMCR_ANENABLE);
                udelay(10);
                netdev_link_down(tp->dev);
            }
