...
void tg3_write_indirect_mbox(struct tg3* tp, u32 off, u32 val)
{
    ...
    if (off == (MAILBOX_RCVRET_CON_IDX_0 + TG3_64BIT_REG_LOW)) {
        pci_write_config_dword(tp->pdev, TG3PCI_RCV_RET_RING_CON_IDX + TG3_64BIT_REG_LOW, val);
        return;
    }
    if (off == TG3_RX_STD_PROD_IDX_REG) {
        pci_write_config_dword(tp->pdev, TG3PCI_STD_RING_PROD_IDX + TG3_64BIT_REG_LOW, val);
        return;
    }
...
static u32 tg3_nvram_phys_addr(struct tg3* tp, u32 addr)
{
    ...
    if (tg3_flag(tp, NVRAM) &&
        ...
        addr = ((addr / tp->nvram_pagesize) << ATMEL_AT45DB0X1B_PAGE_POS) +
...
static void tg3_rings_reset(struct tg3* tp)
{
    ...
    if (tp->rx_rcb) {
        tg3_set_bdinfo(tp, rxrcb, tp->rx_rcb_mapping,
                       TG3_RX_RET_MAX_SIZE_5705 << BDINFO_FLAGS_MAXLEN_SHIFT, 0);
