...
static void t515_reset(struct nic* nic)
{
    ...
    outw(AckIntr | IntLatch | TxAvailable | RxEarly | IntReq,
         nic->ioaddr + EL3_CMD);
    outw(SetIntrEnb | IntLatch | TxAvailable | RxComplete | StatsFull | (vp->bus_master ? DMADone : 0) | UpComplete | DownComplete,
         nic->ioaddr + EL3_CMD);
}
...
static int t515_poll(struct nic* nic, int retrieve)
{
    ...
    if ((cst & RxComplete) == 0) {
        ...
        outw(SetIntrEnb | IntLatch | TxAvailable | RxComplete |
                 StatsFull | (vp->bus_master ? DMADone : 0) | UpComplete |
...
static int t515_probe(struct nic* nic, struct isapnp_device* isapnp) {
    ...
    if ((unsigned)(inw(nic->ioaddr + 0x2002) & 0x1f0) != (nic->ioaddr & 0x1f0)) {
