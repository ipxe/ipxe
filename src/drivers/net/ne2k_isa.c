...
static int ne_probe(struct nic* nic, struct isa_device* isa) {
    ...
    if (eth_vendor == VENDOR_NONE) {
        ...
        outb(D8390_DCR_WTS | D8390_DCR_FT1 | D8390_DCR_LS, eth_nic_base + D8390_P0_DCR);
    ...
    out:
        ...
        DBG("\nNE%c000 base %4.4x, MAC Addr %s\n",
            (eth_flags & FLAG_16BIT) ? '2' : '1', eth_nic_base, eth_ntoa(nic->node_addr));
    }
