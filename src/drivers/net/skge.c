...
static u32 skge_supported_modes(const struct skge_hw* hw)
{
    ...
    if (hw->copper) {
        supported = SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full | SUPPORTED_100baseT_Half | SUPPORTED_100baseT_Full | SUPPORTED_1000baseT_Half | SUPPORTED_1000baseT_Full | SUPPORTED_Autoneg | SUPPORTED_TP;
        ...
        supported = SUPPORTED_1000baseT_Full | SUPPORTED_1000baseT_Half | SUPPORTED_FIBRE | SUPPORTED_Autoneg;
    ...
    if (hw->chip_id == CHIP_ID_GENESIS) {
        ...
        switch (mode) {
            ...
            case LED_MODE_ON:
                ...
                gm_phy_write(hw, port, PHY_MARV_LED_OVER,
                             PHY_M_LED_MO_RX(MO_LED_OFF) |
                                 (skge->speed == SPEED_100 ? PHY_M_LED_MO_100(MO_LED_ON) : 0));
                break;
            case LED_MODE_TST:
                gm_phy_write(hw, port, PHY_MARV_LED_CTRL, 0);
                gm_phy_write(hw, port, PHY_MARV_LED_OVER,
...
static void genesis_stop(struct skge_port* skge)
{
    ...
    xm_write16(hw, port, XM_MMU_CMD,
               xm_read16(hw, port, XM_MMU_CMD) & ~(XM_MMU_ENA_RX | XM_MMU_ENA_TX));
...
static int __gm_phy_read(struct skge_hw* hw, int port, u16 reg, u16* val)
{
    ...
    gma_write16(hw, port, GM_SMI_CTRL,
                GM_SMI_CT_PHY_AD(hw->phy_addr) | GM_SMI_CT_REG_AD(reg) | GM_SMI_CT_OP_RD);
...
static void yukon_reset(struct skge_hw* hw, int port)
{
    ...
    gma_write16(hw, port, GM_RX_CTRL,
                gma_read16(hw, port, GM_RX_CTRL) | GM_RXCR_UCF_ENA | GM_RXCR_MCF_ENA);
}
...
static void yukon_mac_init(struct skge_hw* hw, int port)
{
    ...
    reg = DATA_BLIND_VAL(DATA_BLIND_DEF) | GM_SMOD_VLAN_ENA | IPG_DATA_VAL(IPG_DATA_DEF);
...
static void yukon_stop(struct skge_port* skge)
{
    ...
    gma_write16(hw, port, GM_GP_CTRL,
                gma_read16(hw, port, GM_GP_CTRL) & ~(GM_GPCR_TX_ENA | GM_GPCR_RX_ENA));
...
static void yukon_phy_intr(struct skge_port* skge)
{
    ...
    if (istatus & PHY_M_IS_AN_COMPL) {
        if (gm_phy_read(hw, port, PHY_MARV_AUNE_LP) & PHY_M_AN_RF) {
            reason = "remote fault";
            goto failed;
        }
...
{
    mb();
    return ((ring->to_clean > ring->to_use) ? 0 : NUM_TX_DESC) + (ring->to_clean - ring->to_use) - 1;
}
