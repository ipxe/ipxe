...
static int __gm_phy_read(struct sky2_hw* hw, unsigned port, u16 reg, u16* val)
{
    ...
    gma_write16(hw, port, GM_SMI_CTRL, GM_SMI_CT_PHY_AD(PHY_ADDR_MARV) | GM_SMI_CT_REG_AD(reg) | GM_SMI_CT_OP_RD);
...
static void sky2_phy_init(struct sky2_hw* hw, unsigned port)
{
    ...
    if (sky2_is_copper(hw)) {
        if (!(hw->flags & SKY2_HW_GIGABIT)) {
            ...
            if (sky2->autoneg == AUTONEG_ENABLE && (hw->flags & SKY2_HW_NEWER_PHY)) {
                /* set downshift counter to 3x and enable downshift */
...
static void sky2_rx_stop(struct sky2_port* sky2)
{
    ...
    for (i = 0; i < 0xffff; i++)
        if (sky2_read8(hw, RB_ADDR(rxq, Q_RSL)) == sky2_read8(hw, RB_ADDR(rxq, Q_RL)))
            goto stopped;
...
static int sky2_up(struct net_device* dev)
{
    ...
    if (hw->chip_id == CHIP_ID_YUKON_EC_U && hw->chip_rev == CHIP_REV_YU_EC_U_A0)
...
static void sky2_down(struct net_device* dev)
{
    ...
    if (!(hw->chip_id == CHIP_ID_YUKON_XL && hw->chip_rev == 0 && port == 0 && hw->dev[1]))
...
static int sky2_autoneg_done(struct sky2_port* sky2, u16 aux)
{
    ...
    if (sky2->duplex == DUPLEX_HALF && sky2->speed < SPEED_1000 && !(hw->chip_id == CHIP_ID_YUKON_EC_U || hw->chip_id == CHIP_ID_YUKON_EX))
        sky2->flow_status = FC_NONE;
            ...
            case OP_TXINDEXLE:
                ...
                if (hw->dev[1])
                    sky2_tx_done(hw->dev[1],
                                 ((status >> 24) & 0xff) | (u16)(length & 0xf) << 8);
                break;
...
static int sky2_init(struct sky2_hw* hw)
{
    ...
    switch (hw->chip_id) {
        ...
        case CHIP_ID_YUKON_EC_U:
            hw->flags = SKY2_HW_GIGABIT | SKY2_HW_NEWER_PHY | SKY2_HW_ADV_POWER_CTL;
            break;

        case CHIP_ID_YUKON_EX:
            hw->flags = SKY2_HW_GIGABIT | SKY2_HW_NEWER_PHY | SKY2_HW_NEW_LE | SKY2_HW_ADV_POWER_CTL;
            break;
        ...
        case CHIP_ID_YUKON_FE_P:
            hw->flags = SKY2_HW_NEWER_PHY | SKY2_HW_NEW_LE | SKY2_HW_AUTO_TX_SUM | SKY2_HW_ADV_POWER_CTL;
            break;

        case CHIP_ID_YUKON_SUPR:
            hw->flags = SKY2_HW_GIGABIT | SKY2_HW_NEWER_PHY | SKY2_HW_NEW_LE | SKY2_HW_AUTO_TX_SUM | SKY2_HW_ADV_POWER_CTL;
            break;

        case CHIP_ID_YUKON_UL_2:
            hw->flags = SKY2_HW_GIGABIT | SKY2_HW_ADV_POWER_CTL;
            break;
...
{
    if (sky2_is_copper(hw)) {
        u32 modes = SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full | SUPPORTED_100baseT_Half | SUPPORTED_100baseT_Full | SUPPORTED_Autoneg | SUPPORTED_TP;

        if (hw->flags & SKY2_HW_GIGABIT)
            modes |= SUPPORTED_1000baseT_Half | SUPPORTED_1000baseT_Full;
        return modes;
    } else
        return SUPPORTED_1000baseT_Half | SUPPORTED_1000baseT_Full | SUPPORTED_Autoneg | SUPPORTED_FIBRE;
}
