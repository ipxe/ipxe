...
__vxge_hw_device_get_legacy_reg(struct pci_device* pdev, void __iomem* bar0)
{
    ...
    if (pci_bar_size(pdev, PCI_BASE_ADDRESS_0) == 0x1000000)
        legacy_reg = (struct vxge_hw_legacy_reg __iomem*)(bar0 + 0x800000);
    else
...
__vxge_hw_device_reg_addr_get(struct __vxge_hw_device* hldev)
{
    ...
    for (i = 0; i < VXGE_HW_TITAN_SRPCIM_REG_SPACES; i++) {
        ...
        hldev->srpcim_reg[i] =
            (struct vxge_hw_srpcim_reg __iomem*)(hldev->bar0 + val64);
    }
    ...
    for (i = 0; i < VXGE_HW_TITAN_VPATH_REG_SPACES; i++) {
        ...
        hldev->vpath_reg[i] =
            (struct vxge_hw_vpath_reg __iomem*)(hldev->bar0 + val64);
    }
                           ...
                           struct vxge_hw_device_hw_info* hw_info)
{
    ...
    for (i = 0; i < VXGE_HW_MAX_VIRTUAL_PATHS; i++) {
        ...
        vpmgmt_reg = (struct vxge_hw_vpmgmt_reg __iomem*)(bar0 + val64);
        ...
        if (__vxge_hw_device_access_rights_get(hw_info->host_type,
            ...
            mrpcim_reg = (struct vxge_hw_mrpcim_reg __iomem*)(bar0 + val64);
...
vxge_hw_vpath_strip_fcs_check(struct __vxge_hw_device* hldev, u64 vpath_mask)
{
    ...
    for (i = 0; i < VXGE_HW_MAX_VIRTUAL_PATHS; i++) {
        ...
        for (j = 0; j < VXGE_HW_MAC_MAX_MAC_PORT_ID; j++) {
            if (readq(&vpmgmt_reg->rxmac_cfg0_port_vpmgmt_clone[j]) & VXGE_HW_RXMAC_CFG0_PORT_VPMGMT_CLONE_STRIP_FCS)
                return VXGE_HW_FAIL;
        }
    }
    return status;
                      ...
                      struct __vxge_hw_fifo* fifo)
{
    ...
    fifo->txdl = malloc_phys(sizeof(struct vxge_hw_fifo_txd) * fifo->depth, fifo->depth);
    if (!fifo->txdl) {
        vxge_debug(VXGE_ERR, "%s:%d malloc_phys error\n",
                   __func__, __LINE__);
...
u32 __vxge_hw_vpath_func_id_get(struct vxge_hw_vpmgmt_reg __iomem* vpmgmt_reg)
{
    ...
    return (u32)VXGE_HW_VPATH_TO_FUNC_MAP_CFG1_GET_VPATH_TO_FUNC_MAP_CFG1(val64);
}
    ...
    struct __vxge_hw_virtualpath* vpath)
{
    ...
    for (i = 0; i < VXGE_HW_MAC_MAX_MAC_PORT_ID; i++) {
        val64 = readq(&vpath->vpmgmt_reg->rxmac_cfg0_port_vpmgmt_clone[i]);
        max_pyld =
            (u32)
                VXGE_HW_RXMAC_CFG0_PORT_VPMGMT_CLONE_GET_MAX_PYLD_LEN(val64);
        if (mtu < max_pyld)
            mtu = max_pyld;
    }
...
__vxge_hw_vpath_kdfc_configure(struct __vxge_hw_device* hldev, u32 vp_id)
{
    ...
    vpath->nofl_db =
        (struct __vxge_hw_non_offload_db_wrapper __iomem*)(hldev->kdfc + (vp_id *
