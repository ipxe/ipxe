...
#define FQS(_prefix, _x) \
    (((_x) == 512) ? _prefix##_SIZE_512 : (((_x) == 1024) ? _prefix##_SIZE_1K : (((_x) == 2048) ? _prefix##_SIZE_2K : (((_x) == 4096) ? _prefix##_SIZE_4K : __invalid_queue_size))))
...
gmii_autoneg_advertised(struct efab_nic* efab)
{
    ...
    gmii_advertise = ((falcon_mdio_read(efab, 0, MII_CTRL1000) >> 8) & 0x03);
    ...
    for (count = 0; count < 1000; count++) {
        ...
        if (EFAB_DWORD_FIELD(md_stat, FCN_MD_BSY) == 0) {
            if (EFAB_DWORD_FIELD(md_stat, FCN_MD_LNFL) != 0 ||
                ...
                EFAB_ERR("Error from GMII access " EFAB_DWORD_FMT "\n",
...
efab_poll(struct net_device* netdev)
{
    ...
    while (falcon_event_present(evt)) {
        EFAB_TRACE("Event at index 0x%x address %p is " EFAB_QWORD_FMT "\n", ev_queue->read_ptr,
...
efab_init_mac(struct efab_nic* efab)
{
    ...
    for (count = 0; count < 5; count++) {
        ...
        EFAB_LOG("\n%dMbps %s-duplex\n",
                 (efab->link_options & LPA_EF_10000 ? 10000 : (efab->link_options & LPA_EF_1000 ? 1000 : (efab->link_options & LPA_100 ? 100 : 10))),
                 (efab->link_options & LPA_EF_DUPLEX ? "full" : "half"));
