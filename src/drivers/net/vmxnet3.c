...
static int vmxnet3_transmit(struct net_device* netdev,
    ...
    generation = ((vmxnet->count.tx_prod & VMXNET3_NUM_TX_DESC) ? 0 : cpu_to_le32(VMXNET3_TXF_GEN));
...
static void vmxnet3_poll_tx(struct net_device* netdev) {
    ...
    while (1) {
        ...
        generation = ((vmxnet->count.tx_cons & VMXNET3_NUM_TX_COMP) ? 0 : cpu_to_le32(VMXNET3_TXCF_GEN));
...
static void vmxnet3_refill_rx(struct net_device* netdev) {
    ...
    while (vmxnet->count.rx_fill < VMXNET3_RX_FILL) {
        ...
        generation = ((vmxnet->count.rx_prod & VMXNET3_NUM_RX_DESC) ? 0 : cpu_to_le32(VMXNET3_RXF_GEN));
...
static void vmxnet3_poll_rx(struct net_device* netdev) {
    ...
    while (1) {
        ...
        generation = ((vmxnet->count.rx_cons & VMXNET3_NUM_RX_COMP) ? 0 : cpu_to_le32(VMXNET3_RXCF_GEN));
