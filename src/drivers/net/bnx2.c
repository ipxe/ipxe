...
bnx2_set_link(struct bnx2* bp)
{
    ...
    if (bmsr & BMSR_LSTATUS) {
        ...
        if ((bp->phy_flags & PHY_SERDES_FLAG) &&
            ...
            bnx2_read_phy(bp, MII_BMCR, &bmcr);
            if (!(bmcr & BMCR_ANENABLE)) {
                bnx2_write_phy(bp, MII_BMCR, bmcr | BMCR_ANENABLE);
            }
        }
        bp->phy_flags &= ~PHY_PARALLEL_DETECT_FLAG;
        bp->link_up = 0;
...
bnx2_setup_serdes_phy(struct bnx2* bp)
{
    ...
    if (!(bp->autoneg & AUTONEG_SPEED)) {
        ...
        if ((new_bmcr != bmcr) || (force_link_down)) {
            ...
            if (bp->link_up) {
                bnx2_write_phy(bp, MII_ADVERTISE, adv & ~(ADVERTISE_1000XFULL | ADVERTISE_1000XHALF));
...
bnx2_setup_copper_phy(struct bnx2* bp)
{
    ...
    if (bp->autoneg & AUTONEG_SPEED) {
        ...
        if ((adv1000_reg != new_adv1000_reg) ||
            ...
            bnx2_write_phy(bp, MII_BMCR, BMCR_ANRESTART | BMCR_ANENABLE);
        } else if (bp->link_up) {
...
bnx2_init_chip(struct bnx2* bp)
{
    ...
    if (CHIP_ID(bp) == CHIP_ID_5706_A1)
        ...
        REG_WR(bp, BNX2_HC_CONFIG, BNX2_HC_CONFIG_RX_TMR_MODE | BNX2_HC_CONFIG_TX_TMR_MODE | BNX2_HC_CONFIG_COLLECT_STATS);
    }
