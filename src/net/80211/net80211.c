                            ...
                            uint16_t* net_proto, unsigned int* flags)
{
    ...
    *flags = ((is_multicast_ether_addr(hdr->addr1) ? LL_MULTICAST : 0) |
              (is_broadcast_ether_addr(hdr->addr1) ? LL_BROADCAST : 0));
    return 0;
}
    ...
    for (; ie; ie = ieee80211_next_ie(ie, ie_end)) {
        switch (ie->id) {
            ...
            case IEEE80211_IE_EXT_RATES:
                ...
                for (i = 0; i < ie->len &&
                            dev->nr_rates < NET80211_MAX_RATES;
                     i++) {
                    u8 rid = ie->rates[i];
                    u16 rate = (rid & 0x7f) * 5;
...
int net80211_probe_step(struct net80211_probe_ctx* ctx)
{
    ...
    gather_timeout *= (ctx->essid[0] ? NET80211_PROBE_GATHER : NET80211_PROBE_GATHER_ALL);
    ...
    if (now >= ctx->ticks_channel + ctx->hop_time) {
        dev->channel = (dev->channel + ctx->hop_step) % dev->nr_channels;
        dev->op->config(dev, NET80211_CFG_CHANNEL);
                                  ...
                                  int deauth)
{
    ...
    return net80211_tx_mgmt(dev, deauth ? IEEE80211_STYPE_DEAUTH : IEEE80211_STYPE_DISASSOC, dev->bssid, iob);
}
                                 ...
                                 struct io_buffer* iob, int signal)
{
    ...
    switch (stype) {
        ...
        case IEEE80211_STYPE_DISASSOC:
            ...
            net80211_set_state(dev, is_deauth ? NET80211_AUTHENTICATED : NET80211_ASSOCIATED, 0,
