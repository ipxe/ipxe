...
int ib_push(struct ib_device* ibdev, struct io_buffer* iobuf,
    ...
    grh = (dest->gid_present ? iob_push(iobuf, sizeof(*grh)) : NULL);
...
int ib_pull(struct ib_device* ibdev, struct io_buffer* iobuf,
    ...
    if (payload_len) {
        ...
        *payload_len = ((ntohs(lrh->length) << 2) - (orig_iob_len - iob_len(iobuf)) - pad_len - 4 /* ICRC */);
    }
    ...
    if (qp) {
        if (IB_LID_MULTICAST(dest->lid) && grh) {
            if (!(*qp = ib_find_qp_mgid(ibdev, &grh->dgid))) {
                DBGC(ibdev, "IBDEV %s RX for unknown MGID " IB_GID_FMT "\n", ibdev->name,
    ...
    DBGC2(ibdev, "IBDEV %s RX %04x:%08lx <= %04x:%08lx (key %08x)\n",
          ibdev->name, dest->lid,
          (IB_LID_MULTICAST(dest->lid) ? (qp ? (*qp)->ext_qpn : -1UL) : dest->qpn),
