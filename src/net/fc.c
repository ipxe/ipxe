...
static int fc_xchg_tx(struct fc_exchange* xchg, struct io_buffer* iobuf,
    ...
    fchdr->ox_id = htons((xchg->flags & FC_XCHG_ORIGINATOR) ? xchg->xchg_id : xchg->peer_xchg_id);
    fchdr->rx_id = htons((xchg->flags & FC_XCHG_ORIGINATOR) ? xchg->peer_xchg_id : xchg->xchg_id);
...
static int fc_xchg_rx(struct fc_exchange* xchg, struct io_buffer* iobuf,
    ...
    xchg->peer_xchg_id =
        ntohs((fchdr->f_ctl_es & FC_F_CTL_ES_RESPONDER) ? fchdr->rx_id : fchdr->ox_id);
...
static int fc_port_deliver(struct fc_port* port, struct io_buffer* iobuf,
    ...
    xchg_id = ntohs((fchdr->f_ctl_es & FC_F_CTL_ES_RESPONDER) ? fchdr->ox_id : fchdr->rx_id);
...
int fc_port_login(struct fc_port* port, struct fc_port_id* port_id,
    ...
    if (!fc_link_ok(&port->link)) {
        ...
        DBGC(port, "FCPORT %s logged in via a %s, with local ID "
                   "%s\n", port->name,
             ((port->flags & FC_PORT_HAS_FABRIC) ? "fabric" : "point-to-point link"),
