...
static int tls_ready ( struct tls_connection *tls ) {
	return ( ( ! is_pending ( &tls->client.negotiation ) ) &&
		 ...
		 channel_is_established ( &tls->channel ) );
}
...
static void tls_nonce ( struct tls_connection *tls,
	...
	channel_ephemeral_label ( channel, "tls connection nonce", nonce,
				  sizeof ( *nonce ) );
}
 ...
 * @v tls		TLS connection
 */
static void tls_random_session_id ( struct tls_connection *tls ) {
	struct secure_channel *channel = &tls->channel;

	/* Generate session ID as an ephemeral secret */
	channel_ephemeral_label ( channel, "tls session id", tls->session_id,
...
static int tls_generate_master_secret ( struct tls_connection *tls ) {
	int extended = tls->extended_master_secret;
	int rc;

	/* Generate master secret */
	if ( ( rc = tlskey_master ( &tls->key, extended ) ) != 0 ) {
...
static int tls_generate_keys ( struct tls_connection *tls ) {
	...
	size_t mac_len = tx_cipherspec->suite->mac_len;
	size_t key_len = tx_cipherspec->suite->key_len;
	size_t iv_len = tx_cipherspec->suite->fixed_iv_len;
	uint8_t key[key_len];
	int rc;

	/* Generate TX traffic keys */
	if ( ( rc = tlskey_traffic ( &tls->key, &tls_client,
				     &tls_application ) ) != 0 ) {
		DBGC ( tls, "TLS %p could not generate TX secrets: %s\n",
		       tls, strerror ( rc ) );
		return rc;
	}

	/* Generate TX cipher key material */
	if ( ( rc = tlskey_cipher ( &tls->key, &tls_client, key, key_len,
				    tx_cipherspec->fixed_iv, iv_len,
				    tx_cipherspec->mac_secret,
				    mac_len ) ) != 0 ) {
		DBGC ( tls, "TLS %p could not generate TX keys: %s\n",
		       tls, strerror ( rc ) );
		return rc;
	}

	// HACK
	if ( ( rc = channel_set_cipher ( &tls->channel, &tls->channel.tx,
					 tx_cipherspec->suite->cipher,
					 key, key_len ) ) != 0 ) {
		DBGC ( tls, "TLS %p could not set TX cipher: %s\n",
		       tls, strerror ( rc ) );
		return rc;
	}
	...
	/* Generate RX traffic keys */
	if ( ( rc = tlskey_traffic ( &tls->key, &tls_server,
				     &tls_application ) ) != 0 ) {
		DBGC ( tls, "TLS %p could not generate RX secrets: %s\n",
		       tls, strerror ( rc ) );
		return rc;
	}

	/* Generate RX cipher key material */
	if ( ( rc = tlskey_cipher ( &tls->key, &tls_server, key, key_len,
				    rx_cipherspec->fixed_iv, iv_len,
				    rx_cipherspec->mac_secret,
				    mac_len ) ) != 0 ) {
		DBGC ( tls, "TLS %p could not generate RX keys: %s\n",
		       tls, strerror ( rc ) );
		return rc;
	}

	// HACK
	if ( ( rc = channel_set_cipher ( &tls->channel, &tls->channel.rx,
					 rx_cipherspec->suite->cipher,
					 key, key_len ) ) != 0 ) {
		DBGC ( tls, "TLS %p could not set RX cipher: %s\n",
		       tls, strerror ( rc ) );
		return rc;
	}
	...
	if ( ( rc = cipher_setkey ( rx_cipherspec->suite->cipher,
				    ...
				    key, key_len ) ) != 0 ) {
		DBGC ( tls, "TLS %p could not set RX key: %s\n",
		       tls, strerror ( rc ) );
		return rc;
	}
...
static int tls_psk_save ( struct tls_connection *tls ) {
	...
	if ( ( rc = channel_save ( channel, &session->psid ) ) != 0 ) {
...
static int tls_psk_resume ( struct tls_connection *tls ) {
	...
	/* Resume channel */
	if ( ( rc = channel_load ( channel, &session->psid ) ) != 0 ) {
...
static int tls_select_cipher ( struct tls_connection *tls,
	...
	/* Set key schedule digest algorithm */
	if ( ( rc = tls_set_digest ( tls, suite->handshake ) ) != 0 )
		return rc;
...
static int tls_new_server_hello ( struct tls_connection *tls,
	...
	//
	// HACK HACK HACK add ServerHello before generating keys after
	// potential resumption
	tls_add_handshake ( tls, ( data - 4 ), ( len + 4 ) );
...
static int tls_new_finished ( struct tls_connection *tls,
	...
	if ( ( rc = channel_confirm ( channel, data, len ) ) != 0 ) {
	...
	if ( ( rc = channel_establish ( channel, tls->session->name,
					tls->server.root ) ) != 0 ) {
...
static int tls_new_handshake ( struct tls_connection *tls,
	...
	while ( ( remaining = iob_len ( iobuf ) ) ) {
		...
		//
		// HACK HACK HACK let ServerHello add itself
		if ( handshake->type > TLS_SERVER_HELLO )
