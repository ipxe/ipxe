...
static void iscsi_start_command(struct iscsi_session* iscsi) {
    ...
    DBGC2(iscsi, "iSCSI %p start " SCSI_CDB_FORMAT " %s %#zx\n",
          ...
          (iscsi->command->data_in ? iscsi->command->data_in_len : iscsi->command->data_out_len));
}
...
static int iscsi_rx_scsi_response(struct iscsi_session* iscsi,
                                  const void* data, size_t len,
                                  size_t remaining) {
    struct iscsi_bhs_scsi_response* response = &iscsi->rx_bhs.scsi_response;
    struct scsi_rsp rsp;
...
static int iscsi_rx_login_response(struct iscsi_session* iscsi,
                                   const void* data, size_t len,
                                   size_t remaining) {
    struct iscsi_bhs_login_response* response = &iscsi->rx_bhs.login_response;
    int rc;
...
static int iscsi_rx_data(struct iscsi_session* iscsi, const void* data,
    ...
    struct iscsi_bhs_common_response* response = &iscsi->rx_bhs.common_response;
