...
/** Additional information used for poisoning secrets */
struct channel_poison {
	/** Fixed label */
	char label[16];
	/** Nonce */
	uint32_t nonce;
 ...
 *
 * @v channel		Secure channel
 * @v out		Secret to poison
 * @v len		Length of secret
 *
 * Overwrite a secret with an unpredictable value.  The consumer may
 * use this to poison secrets that would otherwise temporarily be left
 * containing all zeros (or other predictable values).
 */
void channel_poison ( struct secure_channel *channel, void *out,
		      size_t len ) {
	static struct channel_poison poison = {
		.label = "channel poisoned",
	};

	/* Generate from ephemeral master secret and nonce */
	poison.nonce++;
	channel_ephemeral ( channel, &poison, sizeof ( poison ), out, len );
}
...
/**
 * Resume from a pre-shared key
 *
 * @v channel		Secure channel
 * @v psid		Pre-shared bound peer identity
 * @ret rc		Return status code
 */
int channel_resume ( struct secure_channel *channel,
		     struct secure_preshared_identity *psid ) {
	struct x509_certificate *identity;
	int rc;
	...
	/* Resume from pre-shared key */
	if ( ( rc = channel->op->resume ( channel, psid ) ) != 0 ) {
		DBGC ( channel, "CHANNEL %p could not resume pre-shared "
		       "key: %s\n", channel, strerror ( rc ) );
		goto err_resume;
	}
 ...
 *
 * This assertion is an unverifiable promise made by the consumer:
 * there is no way for the secure channel itself to determine how the
 * cipher key is derived.
 */
int channel_set_cipher ( struct secure_channel *channel,
	...
	DBGC ( channel, "CHANNEL %p %s using \"%s\"\n", channel,
	       channel_pipe_name ( channel, pipe ), cipher->name );
	return 0;
...
void channel_close ( struct secure_channel *channel ) {
	...
	/* Reopen secure channel to destroy all existing secrets
	 *
	 * We explicitly choose not to zero the ephemeral master
