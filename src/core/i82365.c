...
int identify_i365(u_short port, u_short sock) {
    ...
    if ((val & PD67_INFO_CHIP_ID) == PD67_INFO_CHIP_ID) {
        ...
        if ((val & PD67_INFO_CHIP_ID) == 0) {
            ...
            if (i365_get(pccsocks, PD67_EXT_INDEX) != 0xe5)
                type = IS_VT83C469;
        }
    }
    return type;
    ...
    if ((id == IS_I82365DF) && (identify_i365(i365_base, 1) != id)) {
        ...
        for (i = 0; i < 4; i += 2) {
            ...
            for (j = ns = 0; j < 2; j++) {
                ...
                if (identify_i365(port, sock + j) < 0)
                    continue;
                ...
                if (k <= i82365s)
                    break;
                add_socket_i365(port, sock + j, id);
                ns++;
            }
        }
    }
...
int i82365_interfacer(interface_func_t func, int sockno, int par1, int par2, void* par3) {
    ...
    switch (func) {
        ...
        case MAPATTRMEM:
            ...
            if (!(1 & i365_get(sockno, I365_ADDRWIN)))
                return 1;
            break;
        case UNMAPATTRMEM:
            i365_set(sockno, I365_ADDRWIN, i365_get(sockno, I365_ADDRWIN) & 0x20);
        ...
        case SELECTCONFIG: // Params: par1: config number; par3 config pointer pointer
            if (0 > pccsock[sockno].configoffset)
                return 1;
            if (NULL == (pccc = par3))
                return 2;
            // write config number to
            upc = ioremap(MAP_ATTRMEM_TO, MAP_ATTRMEM_LEN);
            if (pccsock[sockno].configoffset > MAP_ATTRMEM_LEN)
                return 3;
            if ((par1 & 0x7fffffc0))
                return 4;
            if (pccc->index != par1)
