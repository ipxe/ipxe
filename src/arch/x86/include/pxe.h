...
struct pxe_api_call {
    ...
    #define PXE_API_CALL(_opcode, _entry, _params_type)                                            \
        {                                                                                          \
            .entry = ((((PXENV_EXIT_t(*)(_params_type * params)) NULL) == ((typeof(_entry)*)NULL)) \
                          ? ((PXENV_EXIT_t(*)(union u_PXENV_ANY * params)) _entry)                 \
                          : ((PXENV_EXIT_t(*)(union u_PXENV_ANY * params)) _entry)),               \
...
extern PXENV_EXIT_t pxenv_tftp_read_file(struct s_PXENV_TFTP_READ_FILE* tftp_read_file);
