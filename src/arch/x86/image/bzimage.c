...
static int bzimage_parse_header(struct image* image,
    ...
    bzimg->rm_filesz = (((bzimg->bzhdr.setup_sects ? bzimg->bzhdr.setup_sects : 4) + 1) << 9);
    ...
    is_bzimage = ((bzimg->version >= 0x0200) ? (bzimg->bzhdr.loadflags & BZI_LOAD_HIGH) : 0);
    ...
    bzimg->mem_limit = ((bzimg->version >= 0x0203) ? bzimg->bzhdr.initrd_addr_max : BZI_INITRD_MAX);

    /* Extract command line size */
    bzimg->cmdline_size = ((bzimg->version >= 0x0206) ? bzimg->bzhdr.cmdline_size : BZI_CMDLINE_SIZE);
