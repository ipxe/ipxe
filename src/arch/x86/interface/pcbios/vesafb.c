...
static int vesafb_mode_info(unsigned int mode_number) {
    ...
    DBGC(&vbe_buf, "VESAFB mode %04x %dx%d %dbpp(%d:%d:%d:%d) model "
         ...
         ((mode->mode_attributes & VBE_MODE_ATTR_SUPPORTED) ? "" : " [unsupported]"),
         ((mode->mode_attributes & VBE_MODE_ATTR_TTY) ? " [tty]" : ""),
         ((mode->mode_attributes & VBE_MODE_ATTR_GRAPHICS) ? "" : " [text]"),
         ((mode->mode_attributes & VBE_MODE_ATTR_LINEAR) ? "" : " [nonlinear]"),
         ((mode->mode_attributes & VBE_MODE_ATTR_TRIPLE_BUF) ? " [buf]" : ""));
