...
static void int13_sync_num_drives(void) {
    ...
    num_fdds = ((equipment_word & 0x0001) ? (((equipment_word >> 6) & 0x3) + 1) : 0);
...
static int int13_rw_sectors(struct san_device* sandev,
    ...
    lba = ((((cylinder * int13->heads) + head) * int13->sectors_per_track) + sector - 1);
    count = ix86->regs.al;
        ...
        if (bios_drive != sandev->drive) {
            ...
            if (bios_drive == int13->natural_drive) {
                ...
                return;
                       ...
                       (command == INT13_CDROM_STATUS_TERMINATE) && sandev->is_cdrom) {
...
static int int13_load_eltorito(unsigned int drive, struct segoff* address) {
    ...
    address->segment = (catalog.boot.load_segment ? catalog.boot.load_segment : 0x7c0);
    address->offset = 0;
