...
static void efipci_dma_set_mask(struct dma_device* dma, physaddr_t mask) {
    ...
    op = (is64 ? EfiPciIoAttributeOperationEnable : EfiPciIoAttributeOperationDisable);
...
static int efipci_start(struct efi_device* efidev) {
    ...
    /* Open PCI device */
    if ((rc = efipci_open(device, (EFI_OPEN_PROTOCOL_BY_DRIVER | EFI_OPEN_PROTOCOL_EXCLUSIVE),
                          efipci)) != 0) {
