...
/** Fallback character height */
#define EFIFB_FALLBACK_HEIGHT 16

/** Fallback font
 *
 * Older Apple EFI implementations provide no HII font protocol, which
 * would otherwise make the framebuffer console unusable.  Use an
 * embedded font for all single-byte characters; the HII font (where
 * present) is used only for characters outside this range.
 *
 * Glyph data is the classic VGA 8x16 font as distributed in the Linux
 * kernel (lib/fonts/font_8x16.c, GPL-2.0, generated by cpi2fnt).
 */
static const uint8_t efifb_fallback_font[ 256 * EFIFB_FALLBACK_HEIGHT ] = {
#include "efifb_fallback_font.h"
};
...
static int efifb_draw ( unsigned int character, unsigned int index,
	...
	/* Use the embedded fallback font for single-byte characters:
	 * older Apple EFI provides no HII font protocol, and using
	 * the embedded font unconditionally keeps rendering (and this
	 * code path) identical across all platforms.
	 */
	if ( character <= 0xff ) {
		src = &efifb_fallback_font[ character *
					    EFIFB_FALLBACK_HEIGHT ];
		for ( y = 0 ; ( ( y < EFIFB_FALLBACK_HEIGHT ) &&
				( y < efifb.font.height ) ) ; y++ ) {
			*(glyph++) = ( src[y] ^ toggle );
		}
		return EFIFB_FALLBACK_HEIGHT;
	}

	/* Characters beyond the embedded font require the HII font
	 * protocol.
	 */
	if ( ! efifb.hiifont )
		return -ENOENT;
...
static int efifb_init ( struct console_configuration *config ) {
	...
	/* Locate HII font protocol.  Not all platforms provide it
	 * (older Apple EFI does not); the embedded fallback font
	 * covers single-byte characters without it.
	 */
