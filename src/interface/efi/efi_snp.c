...
static EFI_GUID dummy_load_file_protocol_guid = {
    0x6f6c7323, 0x2077, 0x7523, {0x6e, 0x68, 0x65, 0x6c, 0x70, 0x66, 0x75, 0x6c}};
...
efi_snp_mcast_ip_to_mac(EFI_SIMPLE_NETWORK_PROTOCOL* snp, BOOLEAN ipv6,
    ...
    ip_str = (ipv6 ? "(IPv6)" /* FIXME when we have inet6_ntoa() */ : inet_ntoa(*((struct in_addr*)ip)));
...
efi_snp_transmit(EFI_SIMPLE_NETWORK_PROTOCOL* snp,
    ...
    iobuf = alloc_iob(MAX_LL_HEADER_LEN + ((payload_len > IOB_ZLEN) ? payload_len : IOB_ZLEN));
    if (!iobuf) {
...
static PXE_STATCODE efi_undi_statcode(EFI_STATUS efirc) {
    switch (efirc) {
        ...
        case EFI_UNSUPPORTED:
            return PXE_STATCODE_UNSUPPORTED;
        case EFI_OUT_OF_RESOURCES:
            return PXE_STATCODE_BUFFER_FULL;
...
static int efi_snp_probe(struct net_device* netdev) {
    ...
    efi_snprintf(snpdev->name, (sizeof(snpdev->name) / sizeof(snpdev->name[0])),
                 "%s", netdev->name);
