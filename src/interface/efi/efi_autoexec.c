...
static int efi_autoexec_filesystem(EFI_HANDLE device,
    ...
    if (path) {
        ...
        dirlen = ((((path->Length[1] << 8) | path->Length[0]) - offsetof(typeof(*filepath), PathName)) / sizeof(filepath->PathName[0]));
        for (; dirlen; dirlen--) {
...
static int efi_autoexec_tftp(EFI_HANDLE device) {
    ...
    /* Allocate filename */
    filename_max = (sizeof(packet->Dhcpv4.BootpBootFile) + (sizeof(efi_autoexec_name) - 1 /* NUL */) + 1 /* NUL */);
